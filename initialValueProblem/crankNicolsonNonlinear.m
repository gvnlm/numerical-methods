function xPoints = crankNicolsonNonlinear(f, dfdx, tPoints, x0, ...
...
% dfdx(t, x) - partial derivative of f with respect to x
    ...
    for i = 1:n-1
        ...
        % rearrange crank-nicolson formula such that it is a root finding
        % problem 
        % => xnp1 - xn - (deltaT/2)*(f(tn, xn) + f(tnp1, xnp1)) = 0
        g = @(xnp1) xnp1 - xn - (deltaT./2).*(f(tn, xn) + f(tnp1, xnp1)); % = 0
        dg = @(xnp1) 1 - (deltaT./2).*dfdx(tnp1, xnp1);
